<script lang="ts">
	...
	const fontFamilies = [
		{
			...
			description: 'Used for primary UI elements, body text, and headings for optimal readability.'
		},
		{
			name: 'Serif',
			...
			description: 'Can be used selectively for editorial content or specific display headings where a more traditional feel is desired.'
		},
		{
			name: 'Monospace',
			variable: '--nlds-font-mono',
			value: 'IBM Plex Mono, monospace',
			description: 'Used exclusively for displaying code snippets or tabular data requiring fixed-width characters.'
		}
	];
	...
	const typeScale = [
		...
		},
        {
			...
			lineHeight: 1.6,
			fontFamilyVar: '--nlds-font-sans',
			exampleText: 'Body 01: This is the standard paragraph text used for most content blocks.'
		},
		{
			token: 'nlds-body-02',
			...
			lineHeight: 1.5,
			fontFamilyVar: '--nlds-font-sans',
			exampleText: 'Body 02: Slightly smaller text for secondary information or dense layouts.'
		},
		{
			token: 'nlds-label-01',
	...
	const getFontFamilyValue = (varName: string) => {
		...
		return family ? family.value.split(',')[0].replace(/"/g, '') : 'System Default'; // Return clean primary font name
	}
</script>
...
<section class="page-section">
	<p class="component-description">
		Consistent and legible typography is crucial for a good user experience. The Noetic Labs Design
		System (NLDS) utilizes specific font families: IBM Plex Sans (primary sans-serif), Alegreya (serif),
		and IBM Plex Mono (monospace), accessible via CSS custom properties. This ensures typographic consistency across all applications built with NLDS.
	</p>
</section>
...
<section class="page-section">
	<h2>Font Families</h2>
	<p>
		NLDS defines three core font families accessible via CSS variables. The primary font for most UI text is IBM Plex Sans.
	</p>
	<div class="font-family-list">
		{#each fontFamilies as family}
			<div class="font-family-item">
				<h3>{family.name}</h3>
				<p class="font-family-example" style="font-family: var({family.variable});">
...
<section class="page-section">
	<h2>Type Scale</h2>
	<p>
		A type scale provides predefined typographic styles for headings, paragraphs, labels, and other
		text elements. This ensures visual hierarchy and consistency. <strong>Note:</strong> The specific tokens
		and styles below are WIP (Work In Progress).
	</p>
	<div class="type-scale-table">
		<div class="type-scale-header">
			<div class="type-scale-cell header-token">Token / Tag / Class</div>
			<div class="type-scale-cell header-details">Specs (Size / Weight / Line Height / Family)</div>
		...
		{#each typeScale as style}
			<div class="type-scale-row">
				<div class="type-scale-cell cell-token">
					...
					<CodeSnippet code={style.tag === style.token.split('-')[1] || (style.tag === 'p' && style.token.includes('body')) || (style.tag === 'label' && style.token.includes('label')) || (style.tag === 'code' && style.token.includes('code')) || (style.tag === 'span' && style.token.includes('caption')) ? `<${style.tag}>` : `.${style.token}`} type="inline" />
                </div>
				<div class="type-scale-cell cell-details">
					{style.fontSize} / {style.fontWeight} / {style.lineHeight} / {getFontFamilyValue(style.fontFamilyVar)}
				</div>
				<div class="type-scale-cell cell-example">
					<svelte:element
						this={style.tag}
						class={style.token.startsWith('nlds-') ? style.token : ''}
...
<section class="page-section">
	<h2>Usage</h2>
	<p>
		Apply typography primarily by using semantic HTML elements (like <CodeSnippet code="<h1>" type="inline" />-<CodeSnippet code="<h6>" type="inline" />, <CodeSnippet code="<p>" type="inline" />, <CodeSnippet code="<code>" type="inline" />) which should inherit base styles defined in your global CSS. For more specific control or overrides, use the defined type scale utility classes (e.g., <CodeSnippet code=".nlds-body-02" type="inline" />) or directly reference the font family CSS variables in your stylesheets.
	</p>
...
.editorial-title {
    ...
    <p style="margin-top: 1.5rem;">
        Remember to load the required font families in your application, typically via a link in your HTML head or by importing them into your main CSS file. Ensure you include Alegreya along with IBM Plex Sans and Mono.
    </p>
    <CodeSnippet language="html" type="multi" code={`
...
<style>
    ...
    .header-token, .cell-token { flex-basis: 25%; min-width: 180px; }
    .header-details, .cell-details { flex-basis: 35%; min-width: 250px; color: var(--nlds-text-secondary); }
    .header-example, .cell-example { flex-basis: 40%; flex-grow: 1; min-width: 200px; }
    ...
    .cell-example > * { /* Target the actual example element */
        width: 100%;
	...
	.nlds-heading-01 { font-size: 2rem; font-weight: 600; line-height: 1.4; font-family: var(--nlds-font-sans); }
	.nlds-heading-02 { font-size: 1.75rem; font-weight: 600; line-height: 1.4; font-family: var(--nlds-font-sans); }
	.nlds-heading-03 { font-size: 1.5rem; font-weight: 600; line-height: 1.4; font-family: var(--nlds-font-sans); }
	.nlds-heading-04 { font-size: 1.25rem; font-weight: 600; line-height: 1.5; font-family: var(--nlds-font-sans); }
	.nlds-body-01 { font-size: 1rem; font-weight: 400; line-height: 1.6; font-family: var(--nlds-font-sans); }
	.nlds-body-02 { font-size: 0.875rem; font-weight: 400; line-height: 1.5; font-family: var(--nlds-font-sans); }
	.nlds-label-01 { font-size: 0.75rem; font-weight: 600; line-height: 1.4; font-family: var(--nlds-font-sans); }
	.nlds-code-01 { font-size: 0.875rem; font-weight: 400; line-height: 1.5; font-family: var(--nlds-font-mono); }
	.nlds-caption-01 { font-size: 0.75rem; font-weight: 400; line-height: 1.4; font-family: var(--nlds-font-sans); }
