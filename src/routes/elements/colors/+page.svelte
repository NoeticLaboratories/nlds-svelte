<script lang="ts">
    ...
    const themes = [
        {
            ...
            colors: [
                {
                    name: "--nlds-bg-primary",
                    value: "#ffffff",
                    description: "Primary page background",
                },
                {
                    name: "--nlds-bg-secondary",
                    value: "#f4f4f4",
                    description: "Secondary background (e.g., cards)",
                },
                {
                    name: "--nlds-text-primary",
                    value: "#161616",
                    description: "Primary text color",
                },
                {
                    name: "--nlds-text-secondary",
                    value: "#525252",
                    description: "Secondary text color",
                },
                {
                    name: "--nlds-border-color",
                    value: "#e0e0e0",
                    description: "Default border color",
                },
                {
                    name: "--nlds-link-color",
                    value: "#0f62fe",
    ...
    const utilityClasses = [
        {
            name: ".nlds-accent-bg",
            description: "Applies accent background color and contrast text.",
        },
        {
            name: ".nlds-accent-text",
            description: "Applies accent text color.",
        },
        {
            name: ".nlds-accent-border",
            description: "Applies accent border color.",
        },
    ];
</script>
...
<section class="page-section">
    <p class="component-description">
        The Noetic Labs Design System (NLDS) utilizes a flexible color system
        based on CSS custom properties (variables). This allows for consistent
        theming across applications. The system includes a primary accent color
        and multiple background themes inspired by IBM's <a
...
<section class="page-section">
    <h2>Themes</h2>
    <p>
        NLDS provides multiple themes that control background, text, border, and
        other UI element colors. Themes are applied by setting the <CodeSnippet
            code="data-theme"
            type="inline"
        /> attribute on a container element (often <CodeSnippet
            code="<body>"
            type="inline"
        /> or <CodeSnippet code="<html>" type="inline" />). The default theme is
        White.
    </p>
    ...
    {#each themes as theme}
        <div class="theme-section">
            <h3>{theme.name}</h3>
            <p>
                Apply with: <CodeSnippet
                    code={theme.attribute}
                    type="inline"
                />. Color scheme: <CodeSnippet
                    code={theme.scheme}
                    type="inline"
                />.
            </p>
            <div class="color-grid">
                {#each theme.colors as color}
                    <div class="color-card">
                        <div
                            class="color-swatch"
                            style="background-color: {color.value}; border: 1px solid var(--nlds-border-color, #ccc);"
                        >
                            {#if color.name.includes("text")}
                                <span
                                    class="swatch-text"
                                    style="color: {color.value}; background-color: {theme.colors.find(
                                        (c) => c.name.includes('bg-primary'),
                                    )?.value || '#888'}; padding: 2px 4px;"
                                    >Aa</span
                                >
                            {/if}
                        </div>
...
<section class="page-section">
    ...
    <p>
        For convenience, a few utility classes are provided to directly apply
        accent colors.
    </p>
    <ul>
    ...
    <div class="component-demo utility-demo">
        <div class="nlds-accent-bg demo-box">
            Accent Background (.nlds-accent-bg)
        </div>
        <p class="nlds-accent-text">Accent Text (.nlds-accent-text)</p>
        <div
            class="demo-box nlds-accent-border"
...
<section class="page-section">
    <h2>Usage</h2>
    <p>
        Colors should primarily be consumed via their CSS custom properties
        within your stylesheets or components. This ensures that elements adapt
...
<style>
    ...
    .color-info :global(.ndsl-codesnippet--inline) {
        /* Target CodeSnippet inside */
    ...
    .utility-demo .nlds-accent-bg {
        ...
        color: var(
            --nlds-inverse-01,
            white
        ); /* Use inverse for text on accent */
    }
